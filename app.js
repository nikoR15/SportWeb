...
function filterPrice(event) {
    const responseFilter = event.target.value === 'Menor a 100.00'
    ? products.filter(element => element.price < 100.00)
    : event.target.value === '100.00 - 300.00'
    ? products.filter(element => element.price >= 100.00 && element.price <= 300.00)
    : event.target.value === 'Mayor a 300.00'
    ? products.filter(element => element.price > 300.00)
    : null;
...
function filterCategory(event) {
  const category = event.target.value === "Ropa Deportiva"
  ? products.filter(element => element.genero === "Ropa Deportiva")
  : event.target.value === 'Guayos'
  ? products.filter(element => element.genero === "Guayos")
  : event.target.value === "Raquetas"
  ? products.filter(element => element.genero === "Raquetas")
  : event.target.value === "Balones"
  ? products.filter(element => element.genero === "Balones")
  : event.target.value === "Protección"
  ? products.filter(element => element.genero === "Protección")
  : null;
...
function listProducts() {
  select.innerHTML = '';
  // select.map(element => createProducts(element))
...
import {obtenerProducto} from './firebase.js';

window.addEventListener('DOMContentLoaded', async () => {
  const querySnapshot = await obtenerProducto();
